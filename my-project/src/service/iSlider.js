var iSlider = {
    ...
    _bindEvt:function () {
        ...
        if (this.opts.fullScr || this.opts.preventMove) {
            handlrElm.addEventListener('touchmove', function (e) {e.target.getAttribute('data-stop') !== "true" && e.preventDefault(); }, { passive: false });
        }
    },
...
iSlider.Slider = function (opts) {
    for (var i in opts) {
        iSlider.opts[i]=opts[i];
    }

    iSlider.SS=false;
    try {
        iSlider.SS=sessionStorage;
        iSlider.SS['spt']=1;//检测是否是ios私密浏览模式 如果是私密模式 这一行会报错 进入到catch
    }catch (e) {
        iSlider.SS=0;
    }
